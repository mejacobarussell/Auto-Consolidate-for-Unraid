...
auto_plan_and_execute() {
    ...
    # =====================================================================
    # 2. Scan Verbosity Configuration (Bypassed if fully automated)
    # =====================================================================
    local scan_verbosity_mode=2 # Default to 2 (Minimal output)
    ...
    if [ "$fully_automated_cron" = false ]; then
        ...
        while true; do
            ...
            case "$DISPLAY_SELECTION" in
                1)
                    scan_verbosity_mode=1
                    printf "%b\n" "${YELLOW}Scan set: Verbose mode (Full details).${RESET}"
                    break
                    ;;
                2)
                    scan_verbosity_mode=2
                    printf "%b\n" "${YELLOW}Scan set: Minimal mode (Skip messages hidden, plan shown).${RESET}"
                    break
                    ;;
                3)
                    scan_verbosity_mode=3
                    printf "%b\n" "${YELLOW}Scan set: Progress Only mode (Details hidden).${RESET}"
                    break
                    ;;
                *)
        ...
        # Use default mode 2 (Minimal) for cron job
        printf "%b\n" "${YELLOW}Non-Interactive Mode: Scan verbosity set to Minimal (Mode 2) for log clarity.${RESET}"
    fi
    # =====================================================================
    ...
    # 3. Initialize Disk Free Space and Share Usage for all disks
    ...
    # 4. Iterate through all subdirectories and create the plan
    echo ""
    ...
    # NEW: Initialize folder counter for Mode 3 progress indicator
    ...
    while IFS= read -r full_src_path; do
        ...
        FOLDER_COUNTER=$((FOLDER_COUNTER + 1)) # Increment counter at the start of processing a folder
        ...
        # 4a. Iterate through all disks to find valid candidates
        for d_path in /mnt/{disk[1-9]{,[0-9]},cache}; do
            disk_name="${d_path#/mnt/}"
        ...
        # 4b. Select the best destination disk from the candidates
        BEST_DEST_DISK=""
        ...
        if [ -s "$TEMP_CANDIDATES" ]; then
            ...
            # Print plan item (Mode 1 and 2 only)
            if [ "$scan_verbosity_mode" -ne 3 ]; then 
    ...
    # Add a final newline if the progress indicator was used in Mode 3
    if [ "$scan_verbosity_mode" -eq 3 ]; then
        printf "\n"
    fi
    ...
    # 5. Execute the Plan (WITH PROGRESS INDICATOR)
    echo ""
    printf "%b\n" "${CYAN}--------------------------------------------------------${RESET}"
...
# This block now runs first if no mode flag was supplied AND the share was not set via -L.
    ...
    while true; do
        ...
        case "$MODE_SELECTION" in
            ...
            *)
                ...
                ;;
        # Changed /dev/null to be outside the case structure
        esac 
    done < /dev/tty # Read from terminal input (TTY)
    echo ""
fi

# 2. Prompt for the base share by scanning /mnt/user/
if [ "$share_set_by_arg" = false ]; then
    # Only run the interactive selection if -L was NOT provided
    select_base_share
else
    # If -L was provided, confirm the path
