...
auto_plan_and_execute() {
    ...
    # =====================================================================
    # NEW STEP 1: CONSOLIDATED FOLDER DISPLAY PROMPT (Updated for Mode 3)
    # =====================================================================
    ...
    while IFS= read -r full_src_path; do
        ...
        if [ "$folder_size" -lt 10 ]; then 
            if [ "$scan_verbosity_mode" -eq 3 ]; then
                printf "\r%b%s" "${CYAN}Processing folders: ${BLUE}" "$FOLDER_COUNTER${RESET}"
            fi
            continue 
        fi
        ...
        if is_consolidated "$share_component"; then
            if [ "$scan_verbosity_mode" -eq 1 ]; then
                printf "%b\n" "${YELLOW}  [SKIP]: '${share_component}' (Size: $(numfmt --to=iec --from-unit=1K $folder_size)) - Already consolidated. Skipping calculation.${RESET}"
                 ...
                 # Use \r to overwrite the line for real-time progress
                 printf "\r%b%s" "${CYAN}Processing folders: ${BLUE}" "$FOLDER_COUNTER${RESET}"
            fi
            continue
        fi
